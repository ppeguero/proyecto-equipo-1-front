<template>
  ...
  const localMedicamento = ref<Medicamento>({
    nombre: '',
    descripcion: '',
    dosis: '',
    fechaVencimiento: '',
  ...
  const errors = ref<Partial<Record<keyof Medicamento, string>>>({});
  ...
  const validateForm = async (): Promise<boolean> => {
    try {
      ...
      if (err instanceof Yup.ValidationError) {
        const newErrors: Partial<Record<keyof Medicamento, string>> = {};
        err.inner.forEach((error) => {
          if (error.path) {
            newErrors[error.path as keyof Medicamento] = error.message;
          }
        });
  ...
  watch(
    ...
    (newMedicamento) => {
      if (newMedicamento) {
        localMedicamento.value = { ...newMedicamento };
      } else {
